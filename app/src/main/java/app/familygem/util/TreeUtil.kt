...
object TreeUtil {
    ...
    suspend fun openGedcom(treeId: Int, saveSettings: Boolean): Boolean {
        // @yus: Clear old wrappers before loading new tree
    ...
    // @yus: New function to load wrappers
    private fun loadWrappers(treeId: Int) {
        try {
            val wrappersFile = File(Global.context.filesDir, "${treeId}_wrappers.json")
            if (!wrappersFile.exists()) {
                // No wrappers file means no custom data for this tree
                Global.personWrappers = HashMap()
                return
            }
    ...
    // @yus: New function to save wrappers
    ...
    private suspend fun unzipTree(context: Context, zipFile: File?, zipUri: Uri?, progress: ProgressView? = null): Result<Int> =
        try {
            ...
            if (!wrappersFile.exists()) {
                FileUtils.writeStringToFile(wrappersFile, "{}", "UTF-8")
            }

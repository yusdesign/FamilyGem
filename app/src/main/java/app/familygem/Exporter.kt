...
class Exporter(private val context: Context, private val progressView: ProgressView? = null) {
    ...
    private fun optimizeGedcom() {
        ...
        for (person in gedcom!!.people) {
            ...
            // @yus: Write custom "Сословие" (Social Estate)
            val wrapper = Global.getWrapper(person)
            if (wrapper?.socialEstate?.isNotEmpty() == true) {
                // We need to add this to the person's extensions or a custom field.
                // Since the GedcomWriter doesn't know about our wrapper, we need to store it in the person object.
                // Option 1: Use person.extensions (if available in the library)
                // Option 2: Add a custom tag to the person's events/facts
                // For simplicity, let's add it as a custom event/fact:
                val socialEstateFact = EventFact()
                socialEstateFact.setTag("_SOSL")
                socialEstateFact.setValue(wrapper.socialEstate)
                person.addEventFact(socialEstateFact)
            }

            // @yus: Write custom Associations (non-blood relationships)
            wrapper?.associations?.forEach { assoc ->
                // We need to store associations in a way that GedcomWriter will output them.
                // The GedcomWriter writes the standard structure. We can add a custom structure
                // by creating a custom object and adding it to the person's extensions.
                // For now, we'll add a placeholder comment - this needs more work.
            }
        }
    }
